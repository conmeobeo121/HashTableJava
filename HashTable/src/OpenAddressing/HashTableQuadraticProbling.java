...
public class HashTableQuadraticProbling<K, V> implements Iterable<K> {
	...
	public HashTableQuadraticProbling() {
		this(DEFAULT_CAPACITY, DEFAULT_LOAD_FACTOR);
	}

	public HashTableQuadraticProbling(int capacity) {
		this(capacity, DEFAULT_LOAD_FACTOR);
	}

	// Designated constructor
	public HashTableQuadraticProbling(int capacity, double loadFactor) {
	...
	// Given a number this method find the next
	...
	// Places a key-value pair into the hash-table. If the value already
		...
		do {
			...
			// The current cell already contains a key
				...
				if (j == -1) {
					...
					// Previously seen deleted bucket. Instead of inserting the
					// new element at i where the null element is insert
	...
	public V get(K key) {
		...
		/*Starting at the original hash index quadratically probe until
		we find a spot where out key is or we hit null element in which
		case our element does not exist.*/
		do {
			...
			// We hit a non-null key, perhaps it's the one we're looking for.
				...
				// The key we went is in the hash-table
				if (keyTable[i].equals(key)) {
					...
					/**
					 ...
					 i and j so that the next time we search for this key it will be found
					 faster. This is call lazy deletion/relocation
					 */
					if (j != -1) {
	...
	// NOTE: returns null if the value is null AND else returns
	...
	public V remove(K key) {
		...
		/**
		 ...
		 where out key is or we hit a null element in which case our element
		 does not exist
		 */
	...
	public java.util.Iterator<K> iterator() {
		// Before the iteration begin record the number of modifications
